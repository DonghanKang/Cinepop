  ...
  <div class="container">
	    		...
	    		<tr>
	    			<td>
	    				<div class="product_info_img">
	    					<img src="<c:url value="/resources/images/store/${map['PD_IMAGENAME']} "/>">
	    					<div>
		    					<strong class="product_info_name">${map['PD_NAME'] }</strong>
	    					</div>
	    				</div>
	    			</td>
	    			<td><p><fmt:formatNumber value="${map['PD_PRICE'] }" pattern="#,###"/> 원</p></td>
	    			<td><p>${map['QUANTITY'] }개</p></td>
	    			<td><p><fmt:formatNumber value="${map['PD_PRICE']*map['QUANTITY']}" pattern="#,###"/> 원</p></td>
	    		</tr>
    		</c:forEach>
    	</table>
    </div>
    <div class="t_center">
	    <table class="total_price_wrap">
	    	...
	    	<tr>
	    		<td><strong><h2><fmt:formatNumber value="${totalVal }" pattern="#,###"/></h2> 원</strong></td>
	    		<td class="pay_back_img"><strong><h2>0</h2>원</strong></td>
	    		<td><strong class="red_text"><h2><fmt:formatNumber value="${totalVal-0 }" pattern="#,###"/></h2> 원</strong></td>
	    	</tr>
	    </table>
    </div>
    <!-- <div>
    ...
    <div class="chk_wrap">
    	<form action="<c:url value='/store/payment'/>">
    		<label for="allchk">
    		<input type="checkbox" name="allchk" id="allchk" class="option-input checkbox" > 
			...
			<dl class="gray_agree">
				...
				<dd>
					기프트콘 발송 및 CS처리 등을 이해 수신자로부터 CINEPOP에 수신자의 휴대전화번호를 제공하는 것에 대한 적합한 동의를 받습니다.
				</dd>
				<dt>
